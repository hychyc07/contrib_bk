...
bool CvFaceSalience::configure(yarp::os::Searchable& config){
    ...
    if (!_haarCascade){
		cout << endl << "Unable to read from file " << cascadeFile << ". Trying to prepend ICUB_ROOT/app/..." << endl;
		std::string strICubRoot = yarp::os::getenv("ICUB_ROOT");
		cascadeFile = strICubRoot + "/app/" + cascadeFile;
		_haarCascade = (CvHaarClassifierCascade*)cvLoad(cascadeFile.c_str(), 0, 0, 0 );
		if (!_haarCascade){
			cout << endl << "ERROR: Could not load classifier cascade configuration from " << cascadeFile << endl << endl;
			return false;
		}
		else{
