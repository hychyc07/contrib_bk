...
bool predictors::open(yarp::os::Searchable& config) 
{
    ...
    use_ca = config.check("constaccel", 1, "Use constant aceleration model ? (0/1)").asInt()!=0;
    use_wiener = (bool)config.check("wiener", 1, "Use Wiener model ? (0/1)").asInt()!=0;
    use_singer = (bool)config.check("singer", 1, "Use Singer model ? (0/1)").asInt()!=0;
    use_circular = (bool)config.check("circular", 1, "Use circular model ? (0/1)").asInt()!=0;
    use_curvilin = (bool)config.check("circular", 1, "Use curvilinear model ? (0/1)").asInt()!=0;
    use_cv = (bool)config.check("constvel", 1, "Use constant velocity model ? (0/1)").asInt()!=0;
    use_ballistic = (bool)config.check("ballistic", 1, "Use ballistic model ? (0/1)").asInt()!=0;
    use_correlation = (bool)config.check("correl", 1, "Use correlation (periodic) model ? (0/1)").asInt()!=0;
    _actM[0] = (int)use_ca;
