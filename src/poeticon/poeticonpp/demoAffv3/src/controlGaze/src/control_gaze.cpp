...
Control_GazeModule::Control_GazeModule() : iencTorso(NULL), _mutex(1) {
    for (int i=0; i<8; i++) {
        headpos[i] = 0;
    }
    vergenceGain=0.0;
...
bool Control_GazeModule::configure(yarp::os::ResourceFinder &rf){
	...
	std::string name = rf.check("name",Value("controlGaze"),"module name (string)").asString().c_str();
	setName(("/"+name).c_str()); // modulePortName
    ...
    string robot = rf.check("robot",Value("icub"),"robot name").asString().c_str();
	...
	_bVOR = rf.check("vorON", 0, "use inertial sensor (0/1)").asInt()!=0;
	controlType = controlType | (vorON * (int)_bVOR);
	
	_bLog = rf.check("log", Value(0), "write controller log information? (0/1)").asInt()!=0;
	...
	string strLogFilePath = string(rf.getContextPath().c_str()) + string("/conf/") + strLogFile + string(buf) + string(".txt");
	...
	Property propBoardHead;
    Property propBoardTorso;

	propBoardHead.put("device","remote_controlboard");
	propBoardHead.put("remote",("/"+robot+"/head").c_str());
	propBoardHead.put("local",getName("/head"));

    propBoardTorso.put("device","remote_controlboard");
    propBoardTorso.put("remote",("/"+robot+"/torso").c_str());
    propBoardTorso.put("local",getName("/torso"));

    if(!ddHead.open(propBoardHead)) {
		cout << "Failed to open remote-controlboard (head) with properties: " << std::string(propBoardHead.toString().c_str()) << std::endl;
		return false;
	}
	if(!ddHead.view(ipos)){
    ...
    if(!ddTorso.open(propBoardTorso)) {
		cout << "Not using Torso: failed to open remote-controlboard (torso) with properties: " << std::string(propBoardTorso.toString().c_str()) << std::endl;       
		if(!ddTorso.view(iencTorso)){
			cout << "Torso controlboard opened, but no encoder interface for torso could be retrieved, aborting... " << endl; 
			return false;
		}
    }    
...
//	desverg_oe = 0;
    ...
    */
	...
	double spds[] = {200, 20, 20, 20, 20, 20, 20, 20};
...
bool Control_GazeModule::interruptModule(){
    printf("Control_GazeModule::interruptModule()\n");
	/* 
...
bool Control_GazeModule::updateModule() //this runs every 
{
	...
	if(iencTorso != NULL){
		iencTorso->getEncoders(torsopos);
	}
	...
	Matrix Waist2NeckR(3,3);
	for(int jj=0; jj < 3; jj++)
		for( int kk=0; kk<3; kk++)
			Waist2NeckR(jj,kk)=Waist2Neck(jj,kk);
	Matrix DesRotNeck = NeckNorm * Waist2NeckR * DesRotWaist; 
