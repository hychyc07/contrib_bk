 ...
 * Author: Ugo Pattacini, Vadim Tikhanoff, Ali Paikan
 * email:  ugo.pattacini@iit.it, vadim.tikhanoff@iit.it, ali.paikan@iit.it
...
#define STATE_WAIT              4
#define STATE_OBSERVE           5
#define STATE_FEEDBACK          6
#define STATE_GOHOME            7
#define STATE_RETURN            8
...
protected:
    ...
    BufferedPort<Bottle> inPortCmdString;
    BufferedPort<Bottle> outPortCmdString;
    BufferedPort<Bottle> outPortSpeak;
    BufferedPort<Bottle> inPortEraString;

    yarp::os::RpcClient  outPortParticle;

    BufferedPort<Bottle> inportTrackTarget;
    BufferedPort<Bottle> inportTrackFace;
    BufferedPort<Bottle> inportIMDTargetLeft;
    BufferedPort<Bottle> inportIMDTargetRight;

    BufferedPort<Bottle> statusOutPort;
    Port outportGui;
    Port outportCmdFace;
    ...
    void selectArm()
    {
        ...
        {
            if (state==STATE_REACH || state==STATE_RETURN)
            {    
    ...
    void doObserve()
    {
        if (useLeftArm || useRightArm)
        {
            if (state==STATE_OBSERVE)
            {
                ...
                {
                    fprintf(stdout,"--- Grasping will now => OBSERVE\n");
                    fprintf(stdout,"--- ARM SELECTED IS %d\n", armSel);
                    ...
                    if ( armSel == LEFTARM )
                    {
                        x[0] = -0.28; x[1] = -0.12; x[2] =  0.14;
                        o[0] = -0.490931; o[1] = -0.792355; o[2] =  0.362161; o[3] =  2.650327;
                        fprintf(stdout,"--- OBSERVING WITH LEFT\n");
                    }
                    else if ( armSel == RIGHTARM )
                    {
                        x[0] = -0.26; x[1] = 0.11; x[2] =  0.155;
                        o[0] = -0.288116; o[1] = 0.445777; o[2] =  -0.847509; o[3] =  2.986495;
                        fprintf(stdout,"--- OBSERVING WITH RIGHT\n");
                    }
                    else
                    {
                    ...
                    // if ((Time::now()-latchTimer)>releaseTmo)
                    cartArm->goToPoseSync(x,o);
                    gazeCtrl->lookAtFixationPoint(x);
                    Time::delay(2.0);

                    fprintf(stdout,"--- DONE OBSERVING\n");
                    latchTimer=Time::now();
    ...
    void doGetFeedback()
    {
        ...
        {
            //here should
            // 1 - send command through port for learning init
            if (cmdString.size() > 0)
            {
                Bottle &era = outPortCmdString.prepare();
                era.clear();
                era.addString("field");
                era.addString("a");
                era.addString("a");
                if ( armSel == LEFTARM )
                {
                    fprintf(stdout, "--- READ LEFTARM\n");
                    era.addInt(213);    
                    era.addInt(100);
                }
                else
                {
                    fprintf(stdout, "--- READ RIGHTARM\n");
                    era.addInt(186);    
                    era.addInt(114);;
                
                }                    
                outPortCmdString.write();     
                ...
                if (learn)
                {
                    fprintf(stdout,"IN LEARN\n");
                    Bottle &status = outPortCmdString.prepare();
                    status.clear();
                    status.addString("set");
                    status.addString("speech");
                    status.addString(cmdString.get(1).asString());
                    outPortCmdString.write();
                }
                else
                {
                    fprintf(stdout,"IN WHAT\n");
                    Bottle &status = outPortCmdString.prepare();
                    status.clear();
                    status.addString("what");
                    outPortCmdString.write();

                    Bottle *tmp=inPortEraString.read(true);
                    
                    if (tmp->size() > 0)
                    {
                        
                        fprintf(stdout,"--- READ FROM ERA %s\n", tmp->get(0).asString().c_str());
                        eraReply = tmp->get(0).asString().c_str();
                    }
                ...
                if (learn)
                {
                    Bottle &tmpSpeech1 = outPortSpeak.prepare();
                    tmpSpeech1.clear();
                    //tmpSpeech.addString("this is ");
                    tmpSpeech1.addString("looks nice!!");
                    outPortSpeak.write();
                }
                else
                {
                    string tmp = "I think this is a " + eraReply;
                    Bottle &tmpSpeech1 = outPortSpeak.prepare();
                    tmpSpeech1.clear();
                    //tmpSpeech.addString("this is ");
                    tmpSpeech1.addString(tmp.c_str());
                    outPortSpeak.write();
                }
            }
    ...
    void doGoHome()
    {
        ...
        if (state==STATE_GOHOME)
        {
            if ((Time::now()-latchTimer)>releaseTmo)
            {
                ...
                checkTorsoHome(3.0);
                checkArmHome(LEFTARM,5.0);
                checkArmHome(RIGHTARM,5.0);
                //deleteGuiTarget();
                state=STATE_RETURN;
            }
        }
     ...
     void doReturn() 
     {
        if (useLeftArm || useRightArm)
        {
            //return the ball to user
            if (state == STATE_RETURN)
            {   
                static int cnt = 0;

                if (Bottle *targetPosNew=inportTrackFace.read(false))
                {   
                    
                    if (targetPosNew->size()>0 && (cnt++ > 5))
                    {
                        cnt = 0;
                        Vector fp(3);
                        fp[0]= -0.45;//targetPosNew->get(0).asDouble();
                        fp[1]=targetPosNew->get(1).asDouble();
                        //fp[2]=targetPosNew->get(2).asDouble();
                        fp[2] = 0.30;
                        ...
                        cartArm->goToPositionSync(fp,2.0);
                        printf("POSITION2: (%f, %f, %f)\n", fp[0], fp[1], fp[2]);
                        gazeCtrl->lookAtFixationPoint(fp);
                        cartArm->waitMotionDone();

                        Vector handPos,handOrient;
                        cartArm->getPose(handPos, handOrient);

                        printf("face: (%f, %f, %f)\n", fp[0], fp[1], fp[2]);
                        printf("Hand: (%f, %f, %f)\n", handPos[0], handPos[1], handPos[2]);
       
                        double Ydist = fabs(fp[1]-handPos[1]);
                        double Zdist = fabs(fp[2]-handPos[2]);

                        double disp = 0.0;
                        disp = norm(fp - handPos);
                        
                        printf("DIST: (%f, %f) %f\n", Ydist, Zdist, disp);

                        //if( Ydist < 0.40 && Zdist < 0.40)
                        if( disp  < 0.40 )                       
                        {
                            Bottle cmd,reply;
                            cmd.addString("reset");
                            outPortParticle.write(cmd,reply);
                            printf("asked to reset and reply is %s\n",reply.toString().c_str());
                            openHand();
                            latchTimer=Time::now();
                            state=STATE_WAIT;
                        }
    ...
    void close()
    {
        ...
        inPortCmdString.interrupt();
        inPortCmdString.close();

        inPortEraString.interrupt();
        inPortEraString.close();

        outPortCmdString.close();
        outPortSpeak.close();

        inportTrackFace.interrupt();
        inportTrackFace.close();
...
public:
    ...
    bool threadInit()
    {
        ...
        inportTrackFace.open((name+"/trackFace:i").c_str());

        inPortCmdString.open((name+"/cmdString:i").c_str());
        inPortEraString.open((name+"/era:i").c_str());

        outPortCmdString.open((name+"/cmdString:rpc").c_str());

        outPortSpeak.open((name+"/speak:o").c_str());

        statusOutPort.open((name+"/status:o").c_str());

        outPortParticle.open((name+"/cmdParticle:rpc").c_str());
        ...
        gazeCtrl->blockEyes(5.0);
        gazeCtrl->setSaccadesStatus(0);
    ...
    void getStingCmd()
    {
        if (Bottle *tmp=inPortCmdString.read(false))
        { 
            if (tmp->size() < 2)
            {
                learn = false;
            ...
            fprintf(stdout,"--- OK GOT SOMETHING %s %s \n", tmp->get(0).asString().c_str(), tmp->get(1).asString().c_str());
            cmdString = *tmp;
        }
    ...
    void sendState()
    {
        if ( (state == STATE_OBSERVE) || (state == STATE_FEEDBACK) || (state == STATE_GOHOME) || (state == STATE_RETURN) ) //make sure you send stuff to deactivate all other modules from gyarpbuilder vadim_complex et al. 2013
        {         
            Bottle &status = statusOutPort.prepare();
            status.clear();
            status.addString("running...");
            statusOutPort.write();
        }        
    }
