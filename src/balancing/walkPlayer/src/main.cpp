...
public:
    ...
    bool init()
    {
        ...
        if (drv_ll->isValid() && drv_rl->isValid())
            connected = drv_ll->view(ipos_ll) && drv_ll->view(ienc_ll) && drv_ll->view(ipid_ll) &&
                        drv_rl->view(ipos_rl) && drv_rl->view(ienc_rl) && drv_rl->view(ipid_rl) ;
        else
            connected=false;
        ...
        double speeds[6];
        for (int i=0; i<6; i++) speeds[i] = 20.0;
        ipos_ll->setRefSpeeds(speeds);
        ipos_rl->setRefSpeeds(speeds);
...
public:
    ...
    bool execute_joint_command(int j)
    {
        ...
        {
            driver->ipid_ll->setReferences(ll);
            driver->ipid_rl->setReferences(rl);
        }
        return true;
    }
