...
const double Demo1Module::SAFETY_THR = -0.07;
const double Demo1Module::SAFETY_Z   = 0.0;

const double Demo1Module::OFFSET_R_TOP [] = {0.016, -0.01, 0.041};
const double Demo1Module::OFFSET_L_TOP [] = {0.016,  0.01, 0.041};
...
int Demo1Module::prepare4Grasping(const string & _label, const string & _mode)
{
	...
	if (activeChain != "none")		return 1;
	else
	{
		int done = Reach2Grasp(_label,"",_mode);
		//int done = -8;
		return done;
	}
}
...
int Demo1Module::Reach2Grasp(const string & _label, const string & _side, const string & _mode)
{
	...
	Vector tg = setGraspingTarget(_label,_side);
...
int Demo1Module::Recycle(const string & _label, const string & _mode)
{
	...
	int done = prepare4Grasping(_label,_mode);
