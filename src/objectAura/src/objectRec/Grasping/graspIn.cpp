...
bool graspIn::Init(int arc, char *arv[])
{
	string pRobotName;
...
bool graspIn::Init( string robot_name, double prepose, double endpose, int side)
{
	...
	graspIn::startingJoint = 7;
	graspIn::position_reached = new bool [nj];
...
bool graspIn::move_to_pregrasp_pos()
{
...
bool graspIn::position_move(Vector POS)
{
	pos->setPositionMode();
...
bool graspIn::doGrasp(bool* excludedJoints) {
...
Vector graspIn::get_encs()
{
	encs->getEncoders(positions.data());
	return positions;
}
Vector graspIn::get_velocity()
{
	return velocities;
}
Vector graspIn::get_amps()
{
	amp->getCurrents(amplifier.data());
	return amplifier;
}
Vector graspIn::get_pid()
{
	pid->getErrors(pid_val.data());
	return pid_val;
}

bool* graspIn::get_collisionDetected()
{
	return collisionDetected;
}

bool graspIn::velocity_move(Vector velocity)
{
...
double graspIn::get_prepose_param()
{
	return Prepose_param;
}
double graspIn::get_endpose_param()
{
	return Endpose_param;
}
Vector *graspIn::get_PrePose()
{
	return PrePose;
}
int graspIn::get_joints()
{
	return nj;
}

int graspIn::get_startingJoint()
{
	return startingJoint;
}

int graspIn::checkCurrents()
...
int graspIn::check_encoders(Vector target_positions) {
...
bool graspIn::contactAtEndOfGripping(bool* excludedJoints) {
	// get the newest data
	checkCollisions();
	check_encoders(targetPosition);
...
Vector graspIn::vectorAdd(Vector A, Vector B) {
	int size = A.size();
	/*if (B.size() != size) 
	{
...
Vector graspIn::vectorSubtr(Vector A, Vector B) {
    int size = A.size();
   /* if (B.size() != size) 
	{
...
bool graspIn::setCurrentThreshold(Vector currentThresholds)
{
	current_thresholds=currentThresholds;
	return 0;	
}

bool graspIn::setPidThreshold(Vector PidThresholds)
{
	pid_thresholds=PidThresholds;
	return 0;
}

bool graspIn::setPreposeParam(double pprepose)
{
	Prepose_param=pprepose;
	return 0;
}

bool graspIn::setEndposeParam(double pendpose)
{
