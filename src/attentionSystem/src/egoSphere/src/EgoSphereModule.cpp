...
bool EgoSphereModule::configure(yarp::os::ResourceFinder &rf){
	...
	if(_strControlboard == std::string("not_available")){
		...
		std::cout << endl << endl << "***No server controlboard specified, encoder values will be fixed for zero azimuth and elevation.***" << endl;
	}
	else{
	...
	if(_strTorsoControlboard == std::string("not_available")){
		...
		std::cout << endl << endl << "***No torso server controlboard specified. Torso movement will not be compensated .***" << endl << endl;
	}
	else{
