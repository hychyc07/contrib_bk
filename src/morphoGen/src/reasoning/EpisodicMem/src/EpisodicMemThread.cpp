			...
			if(!request.isNull()) {
			   ...
			   //=================================================================================
				...
				int xid=0;
					for(int i=0;i<NoB;i++)
						{
						  xid = request.get(i+3).asInt();
						  if(xid != 50)
						  {
							 OHub[xid]=1;
						  }
						  cout << "Object ID received" << xid << endl;
						}
                     if(Largeness == 1)
						  {
...
void EpisodicMemThread:: MemControl()
	{
     InitializeAM();
...
int EpisodicMemThread::RememberPast() // loops from the present to partial cue to remembered past
{
