...
typedef struct {
	int n_bin_dis;
	int n_bin_ori_PI;
	int n_bin_ori; // generated from n_bin_ori_PI: 2*n_bin_ori_PI, or n_bin_ori_PI.
	float MAX_range_dis;
	float MAX_range_ori; //generated: 2PI or PI
	int n_pt;
	int n_table;
} ParaQuantize;
typedef struct{
...
private:
	...
	void blurBeforeResize(Mat & im0, Mat & im, float s);
	void oriTrans2RangePi();
