...
CTLdetector::~CTLdetector()
{
	...
	vec_tr_id_prototype.clear();
	vec_tr_which_k.clear();
...
bool CTLdetector::initiate(string yml_file)
{
	...
	if (!fs.isOpened())//if input yml_file doesn't exist, then default valuables
	{
		...
		test_para.canny_thigh = 180.0f;
		test_para.canny_tlow = 90.0f;
		test_para.min_edge_pt = 120;
		test_para.th_overlap_out = 0.30f;
		...
		chamfer_para.k_linear = 0.7143f;//5/7;
		chamfer_para.b_linear = 51.4f;

		quantize_para.n_bin_dis = 4;
		quantize_para.n_bin_ori = 8;
		quantize_para.MAX_range_dis = 12.0f;
		quantize_para.MAX_range_ori = 2*(float)CV_PI;
	...
	if (quantize_para.MAX_range_ori>4)
		be_ori_pi = false; //false means that the orientation is in [-pi pi]
	else
		be_ori_pi = true;
	ptables = NULL;
	printf("***Done.\n");
...
bool CTLdetector::loadParaFileYml(string yml_file)
{
	...
	quantize_para.n_bin_ori = (int)tm["n_bin_ori"];
	quantize_para.MAX_range_dis = (float)tm["MAX_range_dis"];
	quantize_para.MAX_range_ori = (float)tm["MAX_range_ori"];
	...
	tm = fs["cluster_para"];
	cluster_para.K = (int)tm["K"];
	cluster_para.sigma_2 = (float)tm["sigma_2"];
...
bool CTLdetector::writeParaFileYml(string yml_file)
{
	...
	fs << "quantize_para" << "{"
			...
			<< "n_bin_ori" << quantize_para.n_bin_ori
			<< "MAX_range_dis" << quantize_para.MAX_range_dis
			<< "MAX_range_ori" << quantize_para.MAX_range_ori
			<< "}";
	fs << "grouppt_para" << "{"
...
bool CTLdetector::loadTrainFileBin(string bin_file)
{
	...
	if (!infile.good())
	{
		cout<< "ERROR: "<<bin_file<<" doesn't exist.";
		return false;
	}
	...
	infile.read((char* ) &cluster_para.K,sizeof(int));
	for (size_t i=0;i<cluster_para.K;i++){
		infile.read((char* ) &ntmp,sizeof(int));
		vec_tr_id_prototype.push_back(ntmp);
	}
	infile.close();
...
bool CTLdetector::detect(Mat im)
{
	...
	for (i_s=test_para.min_scale;i_s<=test_para.max_scale;i_s++)
	{
		...
		for (i_asp=test_para.min_aspect;i_asp<=test_para.max_aspect;i_asp++)///////////////////
		{
			...
			Mat im_cur_rgb;
			resize(im,im_cur_rgb,Size(),s,s/asp);// Size(cur_w,cur_h));
			if (BETIME)
	...
	if (BETIME)
	{
		...
		printf("  //step3: matches from table\n");
		printf("  //step4: verification with OCMvip\n");
		printf("  //step5: refine the results\n");
		for (int i=0;i<5;i++)
			printf("  -step%d cost time: %.3f s.\n",i+1,t_steps[i]/(double)getTickFrequency());
	}
	if (BETIME_DETAIL)
	{
		printf("\nTotally:\n");
...
int CTLdetector::refineObj(vector<DetObj> & vec_candi_detobj)
{
	...
	//////////////////////////remove this (output all candidates
	/*for (i=0;i<n;i++)
	{
		obj1 = vec_candi_detobj.at(i);
		if (obj1.match_score>1.0f) obj1.match_score=0.99f; //if it is bigger than 1, set to 0.99.
		obj1.mat_edge_NN_tr = vec_mat_tr_edge.at(obj1.id_NN_tr).clone(); //send the edge to this object.
	...
	for (i=0;i<n;i++)
	{
		...
		for (j=0;j<i;j++)
		{
			...
			else
			{
				...
				//if one rect is totally inside the other, remove one
...
/*
 * remove the less edge windows than test_para.min_edge_pt (default: 120)
 */
void CTLdetector::removeLessEdgeWind(Mat im_edge,vector<Point> &pout_lu_pts)
{
	...
	for (i=0;i<n_win_y;i++)
	{
		...
		for (j=0;j<n_win_x;j++)
		{
			...
			if (npt_edge>test_para.min_edge_pt)//output the windows with more edge points
