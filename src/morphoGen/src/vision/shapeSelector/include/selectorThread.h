...
/**
 * @file selectorThread.h
 * @brief Definition of a thread that receives a bottle from input port and sends the corresponding selectors and hubs to the output port.
 */


#ifndef _SELECTOR_THREAD_H_
#define _SELECTOR_THREAD_H_
...
class selectorThread : public yarp::os::RateThread {
private:
    ...
    yarp::os::Bottle  *leftSelector, *rightSelector;
    yarp::os::Semaphore *leftMutex, *rightMutex;
    double selected[20][50];            // matrix to represent the selected portion
...
public:
    ...
    */
    selectorThread(std::string robotname,std::string configFile);
    ...
    */
    void updateSelector(yarp::os::Bottle *queueBottle);
    
    /*
    * function that plots the contents of the cue
    */
    void selectorPlotting(int i);
};

#endif  //_SELECTOR_THREAD_H_
