...
bool GaussianMixture3::saveMuAndSigma(const char fileMu[], const char fileSigma[], 
...
bool GaussianMixture3::loadParams(const char fileName[])
{
...
void GaussianMixture3::debug(void)
{
...
void GaussianMixture3::saveParams(const char filename [])
{
...
void GaussianMixture3::initEM_random(int nState,Matrix DataSet)
{
...
void GaussianMixture3::initEM_kmeans(int nState,Matrix Dataset)
{
  this->nState = nState;
  this->dim = Dataset.ColumnSize();
  sigma = new Matrix[nState];
...
void GaussianMixture3::initEM_TimeSplit(int nState,Matrix DataSet, int offCount)
{
  Vector * mean = new Vector[nState];
  int nData = DataSet.RowSize();
  this->nState = nState;
...
double GaussianMixture3::stepEM(Matrix DataSet, bool update_covariances)
{
  int nData = DataSet.RowSize();
...
void GaussianMixture3::doEM(Matrix DataSet,int blank_run,int max_iter)
{
  /* perform Expectation/Maximization on the given Dataset :
     Matrix DataSet(nSamples,Dimensions).
     The GaussianMixture3 Object must be initialised before 
...
int GaussianMixture3::inverseSigmaMatrices(void)
...
double GaussianMixture3::pdfState(Vector Vin,int state)
{
...
double GaussianMixture3::pdfState(Vector Vin,Vector Components,int state) 
{
...
double GaussianMixture3::distState(Vector Vin,Vector Components,int state) 
{
  /* Compute the probability density function at vector Vin, 
     (given along the dimensions Components), for a given state */ 
  Vector mu_s;
...
Matrix GaussianMixture3::doRegression(Matrix in,Matrix * SigmaOut,Vector inComponents,Vector outComponents)
{
...
Matrix GaussianMixture3::doOffsetRegression( Matrix in,
                                            Vector offset,
                                            Matrix* SigmaOut,
                                            Vector inComponents,
...
Vector GaussianMixture3::getRegressionOffset( Vector in, 
                                             Vector currOut,
                                             Vector inComponents,
                                             Vector outComponents){
    Vector regOut,out;
...
Vector GaussianMixture3::doRegression(Vector in,Matrix& Sigma,
...
long double GaussianMixture3::GaussianPDF(Vector input,Vector Mean,Matrix covariance)
{
  /* get the probability density for a given state and a given vector */
...
double GaussianMixture3::likelihood(Vector input)
{
  double lik=0.;
...
double GaussianMixture3::log_likelihood(Vector input)
{
  return log(likelihood(input));
}

double GaussianMixture3::log_likelihood(Matrix inputs)
{
  double log_lik=0.;
...
Vector GaussianMixture3::doRegressionPartial(Vector in,Matrix& Sigma,
                        Vector inComponents,Vector outComponents,
                        Vector states)
{
  int outDim = outComponents.Size();
...
void GaussianMixture3::addDataPointPartial(const Vector& dataPoint, double lambda, int state, const Vector& inComponents){
...
void GaussianMixture3::addDataPoint(const Vector& dataPoint, double lambda, const Vector& inComponents){
  Vector Pxi(nState);
  Vector in;
...
void GaussianMixture3::addDataSet(const Matrix& dataSet, double lambda, const Vector& inComponents){
...
double GaussianMixture3::stepEM(Matrix DataSet, Vector weights, bool update_covariances)
{
  int nData = DataSet.RowSize();
...
void GaussianMixture3::doEM(Matrix DataSet, Vector weights, int blank_run,int max_iter)
{
  /* perform Expectation/Maximization on the given Dataset :
     Matrix DataSet(nSamples,Dimensions).
     The GaussianMixture3 Object must be initialised before 
...
void GaussianMixture3::balancePriors(){
    for(int i=0;i<dim;i++){
