...
#define XTREE_XMLVER_TAG  3
#define XTREE_COMMENT_TAG 4
#define XTREE_EOF         5
...
pXmlTree  XmlTree::FindByData(string data, int no){
  int cnt = 0;
  for(unsigned int i=0;i<m_SubTrees.size();i++){
    if(data.compare(m_SubTrees[i]->m_Data)==0){
      if(cnt==no)
...
int XmlTree::GetNextTag(ifstream & file, string & str, string & params, string &currStr)
{
  ...
  if(s[0]=='<'){
	...
	str = "";
	return XTREE_XMLVER_TAG;
      }else if(s[1]=='!'){
	str = "";
	return XTREE_COMMENT_TAG;
      }else{
	...
	return XTREE_START_TAG;
      ...
      if(s[1]=='?'){
	cres= XTREE_XMLVER_TAG;
      }else if(s[1]=='!'){
	cres= XTREE_COMMENT_TAG;
      }
...
int XmlTree::LoadFromFile(string filename)
{
  ...
  if(file.is_open()){
    ...
    while((res==XTREE_XMLVER_TAG)||(res==XTREE_COMMENT_TAG)){
      res = GetNextTag(file,s,p,currS);
    }
...
int	XmlTree::LoadFromStream(ifstream & file, string & currStr, string currPath)
{
  ...
  while(!file.eof()){
    ...
    case XTREE_XMLVER_TAG:
      break;
    case XTREE_COMMENT_TAG:
      break;
    case XTREE_EOF:
      return XTREE_ERROR;
    }
...
void XmlTree::Patch(pXmlTree patchTree){
    if(patchTree==NULL) return;
    if(patchTree->GetName()!=m_Name) return;

    m_Data = patchTree->m_Data;

    pXmlTreeList patchParams = patchTree->GetParams();
    for(unsigned int i=0;i<patchParams->size();i++){
        bool bFound = false;
        for(unsigned int j=0;j<m_Params.size();j++){
            if(m_Params[j]->m_Name == patchParams->at(i)->m_Name){
                m_Params[j]->m_Data = patchParams->at(i)->m_Data;
                bFound = true;
                break;
            }
        }
        if(!bFound){
            m_Params.push_back(patchParams->at(i)->Clone());
        }
    }


    for(unsigned int i=0;i<patchTree->m_SubTrees.size();i++){
        pXmlTree subTree = Find(patchTree->m_SubTrees[i]->GetName(),patchTree->m_SubTrees[i]->GetData());
        if(subTree == NULL) subTree = Find(patchTree->m_SubTrees[i]->GetName());
        if(subTree!=NULL){
            subTree->Patch(patchTree->m_SubTrees[i]);
        }else{
            AddSubTree(patchTree->m_SubTrees[i]->Clone());
        }
    }
}
...
bool	XmlTree::ToBool(){
	bool tmp = false;
	tmp = ((m_Data.compare("true")==0||m_Data.compare("yes")==0||m_Data.compare("1")==0)?true:false);
	return tmp;
}
...
bool XmlTree::ParseArguments(int argc, char** argv, pXmlTree validArgsStruct){
    Clear();
    if(argc<0)
        return false;
    SetName("args");
            ...
            if(argv[i][0]=='-'){
                if(argv[i][1]!=0){
                    if(argv[i][1]=='-'){
                        if(argv[i][2]!=0){
                            if(argv[i][2]!='-'){
                                AddSubTree(new XmlTree(argv[i]+2));
                            }else{
                                res = false;
                                //Error
                            }
                        ...
                        AddSubTree(new XmlTree(argv[i]+1));
                    }
                }else{
                    res = false;
                    //Error
                    ...
                    if(argv[i][1]=='-'){
                        if(argv[i][2]!=0){
                            if(argv[i][2]!='-'){
                                ...
                                pXmlTree tree;
                                if((tree=validArgsStruct->Find(option))!=NULL){
                                    if(tree->GetParamValue("needArg").length()>0){
                                        if((i+1<argc)&&(argv[i+1][0]!='-')){
                                            AddSubTree(new XmlTree(tree->GetName(),argv[i+1]));
                                            i++;
                                        }else{
                                            res = false;
                                            //Error
                                        ...
                                        AddSubTree(new XmlTree(tree->GetName()));
                                    }
                                }else{
                                    res = false;
                                    //Error
                        ...
                        pXmlTree tree;
                        if((tree=validArgsStruct->FindByData(option))!=NULL){
                            if(tree->GetParamValue("needArg").length()>0){
                                if((i+1<argc)&&(argv[i+1][0]!='-')){
                                    AddSubTree(new XmlTree(tree->GetName(),argv[i+1]));
                                    i++;
