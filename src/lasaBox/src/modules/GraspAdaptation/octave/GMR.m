function [y, Sigma_y, Priors_y] = GMR(Priors, Mu, Sigma, x, in, out)
%
...
if 0
    ...
    Sigma_y_tmp2 = zeros(length(out),length(out),nbStates);
    for i=1:nbData
        for j=1:nbStates
            Sigma_y_tmp2(:,:,j) = beta(i,j)*(y_tmp(:,i,j)*y_tmp(:,i,j)' + Sigma_y_tmp(:,:,1,j));
        end
...
%beta_tmp = reshape(beta,[1 1 size(beta)]);
%Sigma_y_tmp2 = repmat(beta_tmp, [length(out) length(out) 1 1]) .* repmat(Sigma_y_tmp,[1 1 nbData 1]);
%Sigma_y = sum(Sigma_y_tmp2,4);
