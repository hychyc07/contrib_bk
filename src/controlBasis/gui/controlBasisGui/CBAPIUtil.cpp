...
void CBAPITextWindow::append_text(string text, string tColor) {
    ...
    Glib::RefPtr<Gtk::TextBuffer> refTextBuffer = m_TextView.get_buffer();
    //Gtk::TextBuffer *refTextBuffer = m_TextView.get_buffer();
    int last_line = refTextBuffer->get_line_count();
    
    Glib::RefPtr<Gtk::TextBuffer::Tag> refTagMatch = Gtk::TextBuffer::Tag::create();
    refTagMatch->property_foreground() = tg.c_str();
   
    Gtk::TextBuffer::iterator iter = refTextBuffer->get_iter_at_line(last_line);
    refTextBuffer->insert(iter, text);

    Gtk::TextBuffer::iterator iterStop = refTextBuffer->get_iter_at_line(last_line);
    Gtk::TextBuffer::iterator iterStart  = refTextBuffer->get_iter_at_line(last_line-1);
    //refTextBuffer->apply_tag(tag, iter_old, iter_new);
...
void CBAPITextWindow::prepend_text(string text, string tColor) {
