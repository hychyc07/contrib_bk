...
void ReferenceWindow::startResources() {
    ...
    cout << "referenceGui::startResources():" << endl;
    cout << "\tcartesian: " << runCartRefs << endl;
    cout << "\tconfiguration: " << runConfigRefs << endl;
    cout << "\theading: " << runHeadingRefs << endl;
    cout << endl;
    ...
    if(runCartRefs) {
        cout << "starting cartesian references..." << endl;
        iCubPositionRef->startResource();
        iCubOrientationRef->startResource();    
    }
    if(runHeadingRefs) {
        cout << "starting heading references..." << endl;
        fovea->startResource();
        lpFovea->startResource();
    }
...
void ReferenceWindow::stopResources() {
    ...
    if(resourcesStarted) {
        if(runConfigRefs) {
            cout << "stopping configuration references..." << endl;
        ...
        if(runCartRefs) {
            cout << "stopping cartesian references..." << endl;
            iCubPositionRef->stopResource();
            iCubOrientationRef->stopResource();
        }        
        if(runHeadingRefs) {
            cout << "stopping heading references..." << endl;
            fovea->stopResource();
            lpFovea->stopResource();
        }
...
int main(int argc, char *argv[]) {
    ...
    bool runCarts = true;
    bool runConfigs = true;
    bool runHeadings = true;
    int d = 20;

    for(int i=1; i<argc; i++) {
        if(!strcmp(argv[i],"--cartesian")) {
            runCarts = true;
        }
        if(!strcmp(argv[i],"--configuration")) {
            runConfigs = true;
        }
        if(!strcmp(argv[i],"--heading")) {
            runHeadings = true;
        }
    ...
    mainWindow.runCartesianReferences(runCarts);
    mainWindow.runConfigurationReferences(runConfigs);
    mainWindow.runHeadingReferences(runHeadings);
    mainWindow.setUpdateDelay(d);
    mainWindow.startResources();
