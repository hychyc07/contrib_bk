...
int NonParametricProbabilityDistribution::drawSample(Vector &sample) {
    Vector sample_index(dimension_count);
    if(sorted == 0)
        sort();
...
double NonParametricProbabilityDistribution::getProbability(Vector sample) {
    ...
    bin=0;
    int i;
    for(i=0; i<dimension_count-1; i++)
        bin += (int)(point[i]*pow(no_of_bins[i],dimension_count-1-i));
    bin += point[i];
...
int NonParametricProbabilityDistribution::addSample(Vector sample, double val) {
    ...
    Vector smoothing_sample(dimension_count);
    double v;
...
void NonParametricProbabilityDistribution::setDimensionParameters(Vector min, Vector max, Vector bins) {
    ...
    for(int i=0; i<data_size; i++)
        distribution[i].index.resize(dimension_count);
    ...
    for(int i=0; i<data_size; i++) {
        sorted_distribution[i].index.resize(dimension_count);
        cumulative_distribution[i].index.resize(dimension_count);
    }
...
int NonParametricProbabilityDistribution::loadDistribution(string str) {
    ...
    if(!strcmp(type,"NonParametric")) {
        cout << "found NonParametric Distribution of size = " << dims << endl;
    } else {
        return 0;
    }
