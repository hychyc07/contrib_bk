...
void CB::LogPolarHeadingSquaredError::startPotentialFunction() {
    start();     // mandatory start function
}

void CB::LogPolarHeadingSquaredError::stopPotentialFunction() {
    stop();     // mandatory stop function
}

bool CB::LogPolarHeadingSquaredError::updatePotentialFunction() {
    ...
    //cout << "LogPolarHeadingPF update" << endl; 
    // get data from ports (should be more safety checks...)
    if(inputPorts.size() != 2) {
        cout << "LogPolarHeadingSquaredError::update() -- wrong number of input ports!!" << endl;
        return false;
    }
    ...
    //cout << "LogPolarHeadingSquaredError Potential = " << potential << endl;
    return ok;

}

bool CB::LogPolarHeadingSquaredError::connectToInputs() {
    ...
    if(inputNames.size() != 2) {
        cout << "LogPolarHeadingSquaredError::connectToInputs() -- size mismatch!!" << endl;
        return false;
    }

    cout << "LogPolarHeadingSquaredError::connectToInputs():\n\t" <<
    ...
    cout << "LogPolarHeadingSquaredError::connectToInputs() -- opening current input port..." << endl;
    ok &= inputPorts[0]->open(posCurNameIn.c_str());
    if(!ok) {
        cout << "LogPolarHeadingSquaredError::connectToInputs() -- failed opening current input port..." << endl;
        return ok;
    }

    cout << "LogPolarHeadingSquaredError::connectToInputs() -- opening reference input port..." << endl;
    ...
    cout << "LogPolarHeadingSquaredError::connectToInputs() -- connecting:\n\t" << 
    ...
    if(!ok) {
        cout << "LogPolarHeadingSquaredError::connectToInputs() -- failed connecting to current input pors...\n\n\n" << endl;
        return ok;
    }
    ok &= Network::connect(posRefName.c_str(),posRefNameIn.c_str(),"tcp");
    if(!ok) {
        cout << "LogPolarHeadingSquaredError::connectToInputs() -- failed connecting to reference input pors...\n\n\n" << endl;
        return ok;
    }
    
    cout << "LogPolarHeadingSquaredError done connecting to YARP input ports..." << endl;
