...
bool CB::ConfigurationSquaredError::updatePotentialFunction() {
    ...
    valid = (bool)(b[0]->get(0).asInt()) && (bool)(b[1]->get(0).asInt());

    if(valid) {

        offset = 2;
        int s0 = b[0]->get(1).asInt();
        int s1 = b[1]->get(1).asInt();
...
bool CB::ConfigurationSquaredError::connectToInputs() {
    ...
    string configCurNameIn = "/cb/configuration/squared_error_pf" + tmp0 + "/" + randomIDstr + "/cur/data:i";
    string configRefNameIn = "/cb/configuration/squared_error_pf" + tmp1 + "/" + randomIDstr + "/ref/data:i";
