...
namespace CB {
    ...
    protected:
        ...
        /**
         * the nullspace scaling factor
         **/
        double nullspaceFactor;
    ...
    public:
         ...
         **/
        void addController(std::string sen, std::string ref, std::string pf, std::string eff, double gain_p=1, double gain_i=0, double gain_d=0);
        
        /**
         * add controller method, from names 
         **/
        void addController(std::string sen, std::string pf, std::string eff, double gain_p=1, double gain_i=0, double gain_d=0);
        ...
        /** 
         * sets whether to use P(I)D Controllers or just P(I) Controllers
         **/
        void useDerivativeTerm(bool b) {
            for(unsigned int i=0; i<controllers.size(); i++) {
                controllers[i]->useDerivativeControl(b);
            }
        }

        /** 
         * sets whether to use PI(D) Controllers or just P(D) Controllers
         **/
        void useIntegralTerm(bool b) {
            for(unsigned int i=0; i<controllers.size(); i++) {
                controllers[i]->useIntegralControl(b);
            }
        }
         ...
         **/
        void saveOutput(std::string f);

        /**
         * sets nullspace scaling factor
         **/
        void setNullspaceFactor(double n) { nullspaceFactor = n; }

        /**
         * sets the gains of one or all controllers in law
         **/
        void setGains(double kp, double ki, double kd, bool setAll, int idx=0);

        /**
         * returns whether started has finished and the action is running
         **/
       //bool hasStarted() {
        //    return started;
        // }
