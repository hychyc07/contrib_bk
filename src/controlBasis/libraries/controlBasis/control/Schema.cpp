    ...
    valueDot(0)
{
    ...
    for(int i=0; i<numPrimitiveControllers; i++) {
        ...
        controllers[i]->setGainP(controllerParameters[i].Gain_P);
        controllers[i]->setGainI(controllerParameters[i].Gain_I);
        controllers[i]->setGainD(controllerParameters[i].Gain_D);
    }
...
bool CB::Schema::updateAction() {
    ...
    cout << endl << endl << "SCHEMA[" << schemaName << "] UPDATE, iteration: " << getIterations() << endl << "--------------------------------------------" << endl;
    ...
    if(action < (numPrimitiveControllers+numCompositeControllers)) {
        ...
        for(unsigned int i=0; i<actionStr.size(); i++) {
