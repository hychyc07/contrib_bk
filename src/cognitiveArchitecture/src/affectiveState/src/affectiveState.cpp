 ...
 * 16/01/12  Modified protocol of imageId input to match new proceduralMemory, 
 *           i.e. one image id. and gaze values instead of two; 
 *           this reflects recent changes to the episodicMemory module. DV
 */ 
...
void AffectiveStateThread::run(){
   ...
   while (isStopping() != true) { // the thread continues to run until isStopping() returns true
      ...
      if (isStopping()) {
         if (debug) {
            cout << "AffectiveStateThread::run: aborting" << endl;
         }
