...
class ExecutionModule: public RFModule
{
protected:
    ...
    {
        if (cmd.get(0).asString()=="execute")
        {
            execute(actions.at(cmd.get(1).asInt()-1));
            reply.addString("ack");
            return true;
        }
...
public:
    bool configure(ResourceFinder &rf)
    {
        ...
        string name=rf.check("name",Value("executionModule")).asString().c_str();
