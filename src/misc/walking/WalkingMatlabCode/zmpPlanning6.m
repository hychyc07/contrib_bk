...
zmpy = zeros(1,floor(1*Time_startwalking/T));
% zmpy(end+1:Time_startwalking/T) = spline([floor(0.8*Time_startwalking/T)+1, Time_startwalking/T],  [0 [zmpy(end) YGlobalFootR(1)] 0], [floor(0.8*Time_startwalking/T)+1 : 1 : Time_startwalking/T]);
% Time_startwalking < t
for i= 1:totalnumberofsteps
    ...
    if i==1 % in the first step NO toe off heel strike for stability reason
        if LeftFootMoving(i) % Right foot stance
            % SS
            zmpy( round(TSS_start(i)/T) : round(TSS_end(i)/T) ) = interp1( [TSS_start(i) TSS_end(i)], [zmpy(floor(Time_startwalking/T)) YGlobalFootR(i+1)], [TSS_start(i) : T : TSS_end(i)] );
    ...
    elseif i==totalnumberofsteps
        if LeftFootMoving(i)% Right foot stance
            ...
            % DS
            zmpy( round(TDS_start(i)/T) : round(TDS_end(i)/T) )  = interp1( [TDS_start(i) TDS_end(i)], [ YGlobalFootL(i+1), 0.5*(YGlobalFootR(end)+YGlobalFootL(end)) - 0.01 ]  ,[TDS_start(i) : T: TDS_end(i)] );
