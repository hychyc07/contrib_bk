% Updated 13 April 2010. Keep toe off at the 1st step(NO heel strike) in
% order to avoid rear leg singularity issue. but halve the toe off angle
% evolve from statemanchine3.m, add transition speed at toeoff
% But maybe not necessary now, not started yet
function [WalkingPhase,StartWalkingPhase,EndWalkingPhase,stepnumber]=StateMachine3(WalkingPhase,StartWalkingPhase,EndWalkingPhase,LeftFootMoving, time, stepnumber,TSS_start, TDS_start, TSS_end, TDS_end,  TUA_start, TUA_end,LambdaX)
...
%         Speed(2) = 0*   (Qtoe*pi/180)/( TUA_end(stepnumber-1)-TUA_start(stepnumber-1) );
        ...
        if LeftFootMoving(stepnumber)
			...
			if (stepnumber==2)
				rotY_fl = PercentToe*(Qtoe*pi/180);
