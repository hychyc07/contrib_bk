...
/**
 * \defgroup icub_contrib_module_cable_guard cableLengthGuard
 ...
 *
 * <ul>
 * <li> Ci0 * joint0 + Ci1 * joint1 + Ci2 * joint2 >= di;  i=0, 1, 2 ...
 * </ul>
 *
...
int main(int argc, char *argv[]) 
{
    ...
    if (!options.check("robot") 
        ...
        fprintf(stderr, "Missing either --robot or --part or --command options\n");
        return 0;
    }
	...
	fprintf(stderr, "Opening the robot device!\n");
    Property ddOptions;
    ddOptions.put("device", "remote_controlboard");
    
	string localPortName = "/";
	localPortName += robot.asString();
	localPortName += "/cableChecker/";
	localPortName += part.asString();
	//sprintf(&name[0], "/%s/cableChecker/%s", robot.asString().c_str(), part.asString().c_str());
    ddOptions.put("local", localPortName.c_str());

	string remotePortName = "/";
	remotePortName += robot.asString();
	remotePortName += "/";
	remotePortName += part.asString();
    ...
    cableLengthGuard *cableChecker = new cableLengthGuard(&dd, lastCommand, C, d, 50);

    cableChecker->setThetaMap(thetaMap);

    //Start the port listener
    sprintf(&name[0], "%s", commandPort.asString().c_str());
    ...
    while (!quit) 
        {
            fprintf(stderr, "Type 'quit+enter' to exit the program\n");
            scanf("%s", cmd);
