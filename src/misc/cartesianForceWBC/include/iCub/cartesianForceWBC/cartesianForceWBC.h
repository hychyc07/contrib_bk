...
#ifndef CARTESIAN_FORCE_WBC
#define CARTESIAN_FORCE_WBC
...
public:
    ...
    cartesianForce() {
        ...
        Vd.resize(3,0.0); // Desired vel vector
        Wd.resize(4,0.0); // Desired vel vector
        ...
        ainv.resize(5,5);
        ainv.eye();    
        G.resize(5,0.0); // Desired force vector
    ...
    Matrix computeLAMBDA2(); // Let's do it the Khatib way! (ref [Khatib00])
