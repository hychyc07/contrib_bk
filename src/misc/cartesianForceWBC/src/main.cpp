...
//		printf("Time: %lf\n",t2-t1);
    ...
    bool respond(const Bottle& command, Bottle& reply)
    {
        ...
        else if ((command.get(0).asString()=="press")||(command.get(0).asString()=="p"))
        {
            imp.start2();
            imp.send_cmd = 2;
            reply.addString("[OK]");
        ...
        else if (command.get(0).asString()=="set")
        {
            ...
            else if (command.get(1).asString() == "Vd")
            {
                if (command.size() != 5)
                {
                    reply.addString("Need 5 arguments for Vd");
                    fflush(stdout);
                    return false;
                }
                imp.Vd(0) = command.get(2).asDouble();
                imp.Vd(1) = command.get(3).asDouble();
                imp.Vd(2) = command.get(4).asDouble();
...
//        string model_filename("hrey.xml");
//        imp.model.reset(jspace::test::parse_sai_xml_file(model_filename, true));
