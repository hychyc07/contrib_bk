...
#include "iCub/cartesianForceWBC/cartesianForceWBC.h"
#include "stdio.h"
...
#define N 5
#define MAX_FORCE_Z 2.0

const double THRESHOLD = 3.0;
static const double GRAV_ACC = 9.81;

bool cartesianForce::open(yarp::os::ResourceFinder &rf) {
    ConstString saixml=rf.findFile("saixml");
    printf("Loading SAIxml from file '%s'... ",saixml.c_str());
    ...
    std::string localPorts="/cartesianForceWBC/"+controlled_part;
    ...
    std::string cremotePorts="/";
    cremotePorts+=robotname+"/cartesianController/left_arm";
    coption.put("remote",cremotePorts.c_str());
    ...
    state.position_ = jspace::Vector::Zero(7); // was 5
    state.velocity_ = jspace::Vector::Zero(7); // was 5
    ...
    newDof=curDof;
    // disable the torso
    newDof[0]=0; newDof[1]=0; newDof[2]=0;
    ...
    printf("\nReached ref point, still inside open()...\n");
...
bool cartesianForce::stop() {
    send_cmd=0;
    Vd.zero();
    icart->setTaskVelocities(Vd,Wd);
    printf("\nSet task velocities to zero.\n");
...
void cartesianForce::loop() {
    ...
    if (encs->getEncoders(encoders.data()) == true) {
        ...
        for (int i=0; i<7; i++) // was 5
            state.position_[i] = CTRL_DEG2RAD * encoders(i);
        for (int i=0; i<7; i++)
            state.velocity_[i] = CTRL_DEG2RAD * dq(i);
...
//    jspace::pretty_print(gravity, std::cerr, "jspace::gravity", "  ");
//    jspace::pretty_print(mass_inertia, std::cerr, "jspace::mass_inertia", "  ");

        jspace::Matrix js_ainv;
        model->getInverseMassInertia(js_ainv);
//        jspace::pretty_print(js_ainv, std::cerr, "jspace::ainv", "  ");
//printf("here\n");
        for (int ii=0; ii<5; ii++){
            for (int jj=0; jj<5; jj++){
                ainv[ii][jj] = js_ainv(ii,jj);
            }
        }
//printf("here\n");
    ...
    Matrix Jcut = J_r.submatrix(0,5,0,numberControlledJoints-1);      // remove the non controlled joints
...
//j    printf("X: %.3f,%.3f->%.6f | Y: %.3f,%.3f->%.6f | errZ: %.3f,%.3f->%.6f\n",Fd[0],f_r[0],Fd[0]+f_r[0], Fd[1],f_r[1],Fd[1]+f_r[1], Fd[2],f_r[2],Fd[2]+f_r[2]);
//j    fflush(stdout);

//j//    G = armChain->getTorques().subVector(0, numberControlledJoints-1);

        jspace::Vector js_G;
        model->getGravity(js_G);
//        jspace::pretty_print(js_G, std::cerr, "jspace::js_G", "  ");
//printf("here\n");
        for (int ii=0; ii<5; ii++){
             G[ii] = js_G[ii];
        }
...
//    fprintf(stderr,"J_r: \n%s \n", J_r.toString().c_str());
//    fprintf(stderr,"Jcut: \n%s \n", Jcut.toString().c_str());
    Matrix LAMBDA = pinv(Jcut * ainv * JTcut,1.0e-2);
...
#ifdef SEND_COMMANDS
    if (verbose)
        fprintf(stderr, "(REAL) Trying to send trqs: %s \n", cmdTorques.toString().c_str());
    for (int i = 0; i < numberControlledJoints; i++)
...
#else
    if (verbose)
        fprintf(stderr, "(SIM) Trying to send trqs: %s \n", cmdTorques.toString().c_str());
    for (int i = 0; i < numberControlledJoints; i++)
        if (fabs(cmdTorques.data()[i]) > THRESHOLD)
...
Matrix cartesianForce::computeLAMBDA2() { // Let's do it the Khatib way! (all refs [Khatib00])(more at [Chang99])
    ...
    Vector _l_i_ci(3);
    for (int k=0; k<N; k++) {
        _H_r_i = armChain->getH(k);
        _H_i_r = SE3inv(_H_r_i);
        _H_r_ci = armChain->getCOM(k);
        _H_i_ci = _H_i_r * _H_r_ci;
        _R_i_ci = _H_i_ci.submatrix(0,2,0,2);
        _X_i_ci.setSubmatrix(_R_i_ci,0,0);
        _X_i_ci.setSubmatrix(_R_i_ci,3,3);
        _l_i_ci(0) = _H_r_ci(0,3);
        _l_i_ci(1) = _H_r_ci(1,3);
        _l_i_ci(2) = _H_r_ci(2,3);
    ...
    // Step 2. Inward recursion: Compute L_h_i. 4-1 to 3-0 represented as 3-0.
    ...
    for (int k=N-1; k>-1; k--) {
        ...
        for(int j=k+1;j<N;j++) {  // (ec.20)
            fprintf(stderr,"I_A[%d] = I_A[%d] + L_h_i[%d]*I_A[%d]*L_h_i[%d].transposed()\n",k,k,j,j,j);
            I_A[k] = I_A[k] + L_h_i[j]*I_A[j]*L_h_i[j].transposed();  // (ec.20)
        }
        fprintf(stderr,"I_A[%d]:\n%s\n",k,I_A[k].toString().c_str());
        ...
        L_h_i[k] = X_h_i[k] * (eye6x6 - getS(k)*_S_flat).transposed();  // (ec.21a)
        fprintf(stderr,"L_h_i[%d]:\n%s\n",k,L_h_i[k].toString().c_str());
    }

    // Step 3. Outward recursion: Compute block diagonal starting with OMEGAroot,root
    deque<Matrix> OMEGA;
    OMEGA.push_back(zero6x6);  // OMEGA[0]
    for (int k=0; k<N-1; k++) {  // OMEGA[1,2,3,4]
        OMEGA.push_back(getS(k)*Dinv[k]*getS(k).transposed() + L_h_i[k].transposed()*OMEGA[k]* L_h_i[k]);
        printf("OMEGA[%d]:\n%s\n",k+1,OMEGA[k+1].toString().c_str());
    }

    // Step 4. Outward recursion: Compute block off-diagonal starting with OMEGAroot,root

    // Step 5. Spatial transformation: Compute OMEGAinv_ei_ej from OMEGAi,j.
    OMEGA.push_back(X_h_i[4].transposed()*OMEGA[4]*X_h_i[4]);
    printf("OMEGA[5]:\n%s\n",OMEGA[5].toString().c_str());

    // Step 6. Matrix inversion.
    return pinv(OMEGA[5]);  // LAMBDA is OMEGA[0]'s inverse
}
