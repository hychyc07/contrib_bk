...
static char const * robot_filename;
...
static char const * win_title("WBC iCub left_arm + /trgPort");
...
/*    deFrame locframe;
    ...
    deFrame globframe;
    globframe.multiply(*(tree.info[id5].node->frameGlobal()), locframe);
    fl_line(x0 + (tree.info[id5].node->frameGlobal()->translation()[1] * scale),
	    y0 - (tree.info[id5].node->frameGlobal()->translation()[2] * scale),
	    x0 + (globframe.translation()[1] * scale),
	    y0 - (globframe.translation()[2] * scale));*/ //j//
  }
...
#ifdef OUTPUT_COMS
std::cerr.unsetf(std::ios::floatfield);            // floatfield not set
std::cerr.precision(10);
std::cerr.setf(std::ios::fixed);
//std::cerr << "h[0]: " << tree.info[id1].node->frameGlobal()->translation()[0] << " " << tree.info[id1].node->frameGlobal()->translation()[1] << " " << tree.info[id1].node->frameGlobal()->translation()[2] << "\n";
// PRESENTING AS IDYN
std::cerr << "h[0]: " << tree.info[id2].node->frameGlobal()->translation()[0] << " " << tree.info[id2].node->frameGlobal()->translation()[1] << " " << tree.info[id2].node->frameGlobal()->translation()[2] << "\n";
std::cerr << "h[1]: " << tree.info[id3].node->frameGlobal()->translation()[0] << " " << tree.info[id3].node->frameGlobal()->translation()[1] << " " << tree.info[id3].node->frameGlobal()->translation()[2] << "\n";
std::cerr << "h[2]: " << tree.info[id4].node->frameGlobal()->translation()[0] << " " << tree.info[id4].node->frameGlobal()->translation()[1] << " " << tree.info[id4].node->frameGlobal()->translation()[2] << "\n";
std::cerr << "h[3]: " << tree.info[id5].node->frameGlobal()->translation()[0] << " " << tree.info[id5].node->frameGlobal()->translation()[1] << " " << tree.info[id5].node->frameGlobal()->translation()[2] << "\n";
std::cerr << "h[4]: " << tree.info[id6].node->frameGlobal()->translation()[0] << " " << tree.info[id6].node->frameGlobal()->translation()[1] << " " << tree.info[id6].node->frameGlobal()->translation()[2] << "\n";
std::cerr << "h[5]: " << tree.info[id7].node->frameGlobal()->translation()[0] << " " << tree.info[id7].node->frameGlobal()->translation()[1] << " " << tree.info[id7].node->frameGlobal()->translation()[2] << "\n";
#endif
    deVector3 globpoint;
...
//      scale = w() / 9.0;
  ...
  tick()
  {
	...
	errx(EXIT_FAILURE, "gettimeofday failed");
      }
		...
		double x0, double y0, double scale)
{
  ...
  locframe.translation()[2] = -1.0;
  deFrame globframe;
  globframe.multiply(*(scratch_tree->info[id7].node->frameGlobal()), locframe);
